...
class AMS(nn.Module):
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        # print(top_values_flat)
        # print(threshold_if_in)
        is_in = torch.gt(noisy_values, threshold_if_in)
        threshold_positions_if_out = threshold_positions_if_in - 1
        threshold_if_out = torch.unsqueeze(torch.gather(top_values_flat, 0, threshold_positions_if_out), 1)
        normal = Normal(self.mean, self.std)
        prob_if_in = normal.cdf((clean_values - threshold_if_in) / (noise_stddev + 1e-4))
        prob_if_out = normal.cdf((clean_values - threshold_if_out) / (noise_stddev + 1e-4))
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        #     print(i, expert_outputs[i].shape)
        # output = dispatcher.combine(expert_outputs)
        expert_outputs = [self.experts[i](x)[0] for i in range(len(self.experts))]
        
        output = sum(expert_outputs, 0)
