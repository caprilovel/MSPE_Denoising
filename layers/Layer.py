...
def pad_to_match(a, b, mode='constant', value=0):
    pad = []
    for i in range(len(a.shape)-1, 1, -1):
        pad_size = b.size(i) - a.size(i)
        pad.extend([pad_size // 2, pad_size - pad_size // 2])
    return F.pad(a, pad, mode=mode, value=value)
...
class Transformer_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        intra_out_concat = pad_to_match(intra_out_concat, new_x, mode='constant', value=0)
        inter_out = pad_to_match(inter_out, new_x, mode='constant', value=0)
