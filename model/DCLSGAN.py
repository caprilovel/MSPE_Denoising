...
class DCLSGAN(nn.Module):
    def __init__(self, in_channels=15, latent_dim=100):
        super().__init__()
        ...
        class Generator(nn.Module):
            def __init__(self):
                ...
                self.decoder = nn.Sequential(
                    nn.Conv1d(128 + 256, 256, 15, padding=7),
                    nn.BatchNorm1d(256),
                    nn.ReLU(),
                    nn.ConvTranspose1d(256, 128, 15, padding=7),
                    nn.BatchNorm1d(128),
                    nn.ReLU(),
                    nn.ConvTranspose1d(128, in_channels, 15, padding=7),
                    nn.Tanh()
                )

            def forward(self, noise, condition):
                # condition shape: (B, C, L)
                # noise shape: (B, latent_dim)
                noise_feat = self.noise_proj(noise).unsqueeze(-1)  # (B, 256, 1)
                noise_feat = noise_feat.expand(-1, -1, condition.size(2))  # (B, 256, L)
                
                cond_feat = self.condition_encoder(condition)  # (B, 128, L)
                
                combined = torch.cat([cond_feat, noise_feat], dim=1)  # (B, 128+256, L)
                return self.decoder(combined)  # (B, C, L)
        ...
        class Discriminator(nn.Module):
            def __init__(self):
                ...
                self.signal_encoder = nn.Sequential(
                    nn.Conv1d(in_channels, 128, 15, padding=7),
                    nn.BatchNorm1d(128),
                    nn.LeakyReLU(0.2),
                ...
                self.joint_net = nn.Sequential(
                    nn.Conv1d(128 + 256, 512, 15, padding=7),
                    nn.BatchNorm1d(512),
                    nn.LeakyReLU(0.2),
            ...
            def forward(self, signal, condition):
                ...
                # condition shape: (B, C, L)
                cond_feat = self.condition_encoder(condition)  # (B, 128, L)
                sig_feat = self.signal_encoder(signal)  # (B, 256, L)
                combined = torch.cat([cond_feat, sig_feat], dim=1)  # (B, 384, L)
                return self.joint_net(combined)  # (B, 1)
